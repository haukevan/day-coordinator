...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (!eventVendor) return NextResponse.json({ error: "Invalid invite link." }, { status: 404 });
  if (eventVendor.inviteTokenExpiresAt && eventVendor.inviteTokenExpiresAt < new Date()) {
    return NextResponse.json({ error: "This invite link has expired. Ask the coordinator to resend it." }, { status: 410 });
  }
  if (eventVendor.status === "ACCEPTED") {
    return NextResponse.json({ eventId: eventVendor.eventId, alreadyAccepted: true });
  }

  // Verify the authenticated user's email matches the invite
  if (dbUser.email.toLowerCase() !== eventVendor.vendorContact.email.toLowerCase()) {
